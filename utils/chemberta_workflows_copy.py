...
def train_chemberta_multilabel_model(
    ...
    val_macro_f1 = evaluate_per_label_metrics(
        trainer,
        val_dataset,
        target_cols,
        threshold=threshold,
    ...
    # Test set evaluation: for final reporting only (do NOT use for tuning)
    print("\nEvaluating model on test set...")
    metrics = trainer.evaluate(eval_dataset=test_dataset)
    test_macro_f1 = evaluate_per_label_metrics(
        trainer,
        test_dataset,
        target_cols,
        threshold=threshold,
    ...
    complete_results = {
        ...
        "metrics": metrics,
        # validation-set metrics (for hyperparameter selection)
    ...
    # Backwards compatible: second return value is still test macro-F1
    return complete_results, test_macro_f1


def grid_search_gamma_alpha(
    args,
    df_train,
    df_test,
    df_val,
    ...
    """
    Grid search over FocalLoss gamma and alpha.

    IMPORTANT: model selection is done using *validation* macro-F1,
    never test-set performance.
    """
    if gammas is None:
    ...
    for g in gammas:
        for a in alphas:
            print(f"\n==== Grid search: gamma={g}, alpha={a} ====")

            complete_results, _ = train_chemberta_multilabel_model(
                args=args,
                df_train=df_train,
                df_test=df_test,
                df_val=df_val,
            ...
            val_f1 = complete_results["val_macro_f1"]
            test_f1 = complete_results["test_macro_f1"]
            ...
            results.append(
                {
                    ...
                    "output_dir": complete_results["output_dir"],
                }
            )

            # Model selection based ONLY on validation macro-F1
            if val_f1 > best_val_f1:
                best_val_f1 = val_f1
                best_config = (g, a)
                best_output = complete_results

    print("\n================ GRID SEARCH SUMMARY ================")
    print(
        f"Best config (by validation): gamma={best_config[0]}, alpha={best_config[1]}"
    )
    print(f"Best validation macro-F1: {best_val_f1}")
    print("=====================================================")

    return results, best_output
