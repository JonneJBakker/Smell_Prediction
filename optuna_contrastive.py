...
class ArgsForTraining:
    ...
    def __init__(self):
        ...
        self.gamma = None
        self.alpha = None
        self.pooling_strat = 'mean_pooling'# "mean_pooling", "cls", or "attention"

        #contrastive loss params:
        self.use_contrastive = True
        self.contrastive_weight = None
...
def make_objective(cli_args):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        threshold = trial.suggest_float("threshold", 0.25, 0.37)

        # focal loss hyperparameters
        gamma = trial.suggest_float("gamma", 1.75, 2.25)
        alpha = trial.suggest_float("alpha", 0.2, 0.45)  # scalar alpha ∈ [0,1]

        #Contrastive params

        contrastive_weight = trial.suggest_float("contrastive_weight", 0.05, 0.30)
        contrastive_temperature =trial.suggest_float("contrastive_temperature", 0.05, 0.3)
        ...
        args.gamma = gamma
        args.alpha = alpha
        args.pooling_strat = "mean_pooling"

        args.use_contrastive = True
        args.contrastive_weight = contrastive_weight
        args.contrastive_temperature = contrastive_temperature
